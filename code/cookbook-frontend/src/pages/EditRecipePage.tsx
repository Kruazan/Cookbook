...
interface RecipeData {
    ...
    steps: {
        processId: number;
        duration: number;
        stepIngredients: { ingredientId: number; quantity: number }[];
    }[];
}
...
const EditRecipePage = () => {
    ...
    useEffect(() => {
        const loadRecipe = async () => {
            try {
                ...
                setName(recipe.name);
                setWeight(recipe.weight.toString());
                setCalories(recipe.calories.toString());

                // Преобразуем шаги
                const loadedSteps: RecipeStep[] = recipe.steps.map((step, index) => {
                    const process = processes.find(p => p.id === step.processId);
                    if (!process) throw new Error(`Process ${step.processId} not found`);

                    const stepIngredients = step.stepIngredients.map(si => {
                        const ingredient = ingredients.find(i => i.id === si.ingredientId);
                        if (!ingredient) throw new Error(`Ingredient ${si.ingredientId} not found`);
                        return {
        ...
        if (processes.length > 0 && ingredients.length > 0 && id) {
            loadRecipe();
        }
    }, [id, processes, ingredients]);
        ...
        return (
            <MainLayout>
                <Box sx={{ p: 3, textAlign: 'center' }}>
                    <CircularProgress />
                    <Typography sx={{ mt: 2, color: '#666' }}>Загрузка рецепта...</Typography>
                </Box>
            </MainLayout>
        );
    }
    ...
    if (error) {
        return (
            <MainLayout>
                <Box sx={{ p: 3, textAlign: 'center' }}>
                    <Typography color="error">Не удалось загрузить рецепт</Typography>
